...
comment 'Synchronize household and member data from ubr_csv_imports into households, individuals, and locations'
BEGIN
...
,uci.district_code
,uci.village_code
, uci.household_ml_code /* Refer to trigger definition generateHouseholdMlCode. */
,uci.form_number
,'n/a'
,uci.floor_type
,uci.roof_type
...
,uci.batch_number
,uci.group_village_head_name
,uci.group_village_head_code
,uci.village_name
FROM distinct_ubr_csv_import_household_records_v uci
WHERE (uci.data_import_id = _data_import_id and uci.archived = false and uci.validation_status = 'Valid')
ON duplicate KEY
UPDATE
...
,households.group_village_head_name = uci.group_village_head_name
,households.group_village_head_code = uci.group_village_head_code
,households.village_name = uci.village_name;
...
, did.household_id
, (select ml_code from households where household_id = t1.household_id limit 1)
...
WHERE (t1.data_import_id = _data_import_id and t1.archived = false and t1.validation_status = 'Valid')
...
/* Import locations */
/**
 * First we insert locations, which then gets assigned IDs.
 * After that, we link the locations using their respective codes
 */
...
WITH locations_to_import AS (
/*
 * 	Transpose records based on the following structure:
 *
 *  District > TA > VC > Zone > Village
 *
 * */
	SELECT district_code AS code
		, district_name AS name
		, "SUBNATIONAL1" AS location_type
		, 0 AS parent_code
		, data_import_id
	FROM distinct_ubr_csv_import_household_records_v
	WHERE data_import_id = _data_import_id AND archived = FALSE AND validation_status = 'Valid'
	UNION
	SELECT ta_code AS code
		, traditional_authority_name AS name
		, "SUBNATIONAL2" AS location_type
		, district_code AS parent_code
		, data_import_id
	FROM distinct_ubr_csv_import_household_records_v
	WHERE data_import_id = _data_import_id AND archived = FALSE AND validation_status = 'Valid'
	UNION
	SELECT cluster_code AS code
		, cluster_name AS name
		, "SUBNATIONAL3" AS location_type
		, ta_code AS parent_code
		, data_import_id
	FROM distinct_ubr_csv_import_household_records_v
	WHERE data_import_id = _data_import_id AND archived = FALSE AND validation_status = 'Valid'
	UNION
	SELECT zone_code AS code
		, zone_name AS name
		, "SUBNATIONAL4" AS location_type
		, cluster_code AS parent_code
		, data_import_id
	FROM distinct_ubr_csv_import_household_records_v
	WHERE data_import_id = _data_import_id AND archived = FALSE AND validation_status = 'Valid'
	UNION
	SELECT village_code AS code
		, village_name AS name
		, "SUBNATIONAL5" AS location_type
		, zone_code AS parent_code
		, data_import_id
	FROM distinct_ubr_csv_import_household_records_v
	WHERE data_import_id = _data_import_id AND archived = FALSE AND validation_status = 'Valid'
)
SELECT
	locations_to_import.code
	,locations_to_import.name
	,locations_to_import.location_type
	,locations_to_import.parent_code
	,locations_to_import.data_import_id
	, CURRENT_TIMESTAMP()
	, TRUE
FROM locations_to_import
ON DUPLICATE KEY UPDATE
locations.name = locations_to_import.name
,locations.tmp_parent_code  = locations_to_import.parent_code
,locations.tmp_import_id = locations_to_import.data_import_id;

/* Resolve parent_id links */
UPDATE locations l1
JOIN locations l2 ON l2.code = l1.tmp_parent_code
SET l1.parent_id = l2.id
WHERE l1.tmp_import_id = _data_import_id;

-- update household member codes
