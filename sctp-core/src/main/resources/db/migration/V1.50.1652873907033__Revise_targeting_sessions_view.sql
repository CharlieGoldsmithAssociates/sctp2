...
SELECT ts.*
	...
	, (SELECT EXISTS (SELECT 1 FROM targeting_results tr2 WHERE tr2.targeting_session = ts.id AND tr2.scm_user_id IS NOT NULL)) AS app_scm
	, (SELECT EXISTS (SELECT 1 FROM targeting_results tr3 WHERE tr3.targeting_session = ts.id AND tr3.dm_user_id  IS NOT NULL)) AS app_dm
