...
CREATE PROCEDURE createEnrollmentSessionFromCbtOrPev(IN _targeting_session_id bigint, IN _verification_session_id bigint, IN _user_id bigint)
    COMMENT 'This procedure creates an enrollment session, then copies households from the given pre-eligibility verification or targeting session'
BEGIN
   ...
   START TRANSACTION;
	...
	IF _targeting_session_id IS NOT NULL AND _targeting_session_id > 0 THEN
		...
		SELECT @enrollment_session_id, tr.household_id, 'Selected', @curr_timestamp, @curr_timestamp, NULL, NULL
		FROM targeting_results tr WHERE tr.targeting_session = _targeting_session_id AND tr.status = 'Selected';
	...
	ELSEIF _verification_session_id IS NOT NULL AND _verification_session_id > 0 THEN
		...
		INSERT INTO household_enrollment
		(session_id, household_id, status, created_at, updated_at, reviewer_id, reviewed_at)
		SELECT @enrollment_session_id, eh.household_id, 'Eligible', @curr_timestamp, @curr_timestamp, NULL, NULL
		FROM eligible_households eh WHERE eh.session_id = _verification_session_id AND eh.selection_status = 'PreEligible';
	END IF;
